...
export function Fretboard({ root, box, frets, blues, intervals, hidden, connect, pathString, sounding, onPlay, quiz }: FretboardProps) {
  const boxNum = box === 'all' ? -1 : +box;
  return (
    <div className="board-scroll" tabIndex={0} aria-label="Scroll fretboard horizontally">
      <div className="fretboard" style={{ gridTemplateColumns: `34px repeat(${frets.length},minmax(43px,1fr))` }}>
        <span />
        ...
        {TUNING.map((_, s) => (
          <div className="string-row" key={s}>
            ...
            {frets.map(f => {
              ...
              const primary = boxNum < 0 || inPosition(s, f, root, boxNum, blues);
              ...
              const cls = ['note', d === 0 && 'root', d === 6 && 'blue', !primary && !isTarget && 'neighbor',
